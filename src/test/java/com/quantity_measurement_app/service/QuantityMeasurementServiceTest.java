...
public class QuantityMeasurementServiceTest {
	...
	void testService_CompareEquality_DifferentUnit_Success() {
		QuantityDTO q1 = new QuantityDTO(10.0, "FEET", "LENGTHUNIT");
		QuantityDTO q2 = new QuantityDTO(120.0, "INCH", "LENGTHUNIT");
		assertTrue(service.compare(q1, q2));
	}
	...
	void testMultiplication() {
		QuantityDTO q1 = new QuantityDTO(10.0, "METER", "LENGTHUNIT");
		QuantityDTO q2 = new QuantityDTO(2.0, "METER", "LENGTHUNIT");
		assertEquals(20.0, service.multiply(q1, q2).getValue());
	}
	...
	@Test
	void testLengthUnits_FromFrontend_AllSupported() {
		List<String> units = List.of("MILLIMETRE", "CENTIMETERS", "INCHES", "FEET", "YARDS");
		for (String unit : units) {
			QuantityDTO quantity = new QuantityDTO(2.0, unit, "LENGTHUNIT");
			assertDoesNotThrow(() -> service.convert(quantity, "INCHES"));
		}
	}

	@Test
	void testWeightUnits_FromFrontend_AllSupported() {
		List<String> units = List.of("MILLIGRAM", "GRAM", "KILOGRAM", "OUNCE", "POUND", "TONNE");
		for (String unit : units) {
			QuantityDTO quantity = new QuantityDTO(2.0, unit, "WEIGHTUNIT");
			assertDoesNotThrow(() -> service.convert(quantity, "GRAM"));
		}
	}

	@Test
	void testVolumeUnits_FromFrontend_AllSupported() {
		List<String> units = List.of("MILLILITRE", "LITRE", "GALLON", "CUBIC_METRE");
		for (String unit : units) {
			QuantityDTO quantity = new QuantityDTO(2.0, unit, "VOLUMEUNIT");
			assertDoesNotThrow(() -> service.convert(quantity, "LITRE"));
		}
	}

	@Test
	void testTemperatureUnits_FromFrontend_AllSupported() {
		List<String> units = List.of("CELSIUS", "FAHRENHEIT", "KELVIN");
		for (String unit : units) {
			QuantityDTO quantity = new QuantityDTO(-10.0, unit, "TEMPERATUREUNIT");
			assertDoesNotThrow(() -> service.convert(quantity, "CELSIUS"));
		}
	}

	@Test
	void testGetAllMeasurements_IsScopedToCurrentUser() {
		SecurityContextHolder.getContext()
				.setAuthentication(new UsernamePasswordAuthenticationToken("user1@test.com", null,
						List.of(new SimpleGrantedAuthority("ROLE_USER"))));
		when(mockRepository.findByUserEmail("user1@test.com")).thenReturn(List.of(new QuantityMeasurementEntity()));

		service.getAllMeasurements();

		verify(mockRepository).findByUserEmail("user1@test.com");
		SecurityContextHolder.clearContext();
	}

	@Test
	void testGetMeasurementsByOperation_IsScopedToCurrentUser() {
		SecurityContextHolder.getContext()
				.setAuthentication(new UsernamePasswordAuthenticationToken("user2@test.com", null,
						List.of(new SimpleGrantedAuthority("ROLE_USER"))));
		when(mockRepository.findByOperationAndUserEmail("ADD", "user2@test.com"))
				.thenReturn(List.of(new QuantityMeasurementEntity()));

		service.getMeasurementsByOperation("ADD");

		verify(mockRepository).findByOperationAndUserEmail("ADD", "user2@test.com");
		SecurityContextHolder.clearContext();
	}
}
